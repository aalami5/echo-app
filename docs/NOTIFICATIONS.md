...
| `/notify/meeting-reply` | POST | Calendar-checked reply card |
...
Message notifications can include an optional `card` payload. The server stores the full card in `pending-messages.json` and sends a compact card in push data to stay inside APNs limits. Build 67 uses `card.type = "meeting_reply"` for scheduling replies generated by `/notify/meeting-reply`; the app hydrates the card through tap handling, missed-notification sync, or server queue sync and renders it in chat with a copy-ready reply action.

The delivery ledger at `notification-deliveries.json` is a local diagnostic record, separate from Supabase notification acknowledgements. App message sync writes `acked` events so missed-delivery investigations can distinguish queued, synced, and acknowledged notifications.
