...
Message notifications can include an optional `card` payload. The server stores the full card in `pending-messages.json` and sends a compact card in push data to stay inside APNs limits. Build 66 uses `card.type = "meeting_reply"` for scheduling replies generated by `/notify/meeting-reply`; the app hydrates the card through tap handling, missed-notification sync, or server queue sync and renders it in chat.
