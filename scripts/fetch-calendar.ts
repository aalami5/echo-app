 ...
 * 
 * Usage: npx ts-node scripts/fetch-calendar.ts [--today|--tomorrow|--week]
 */
...
function detectVideoProvider(url: string): 'zoom' | 'teams' | 'meet' | 'webex' | 'other' {
...
function extractVideoFromDescription(description: string): { videoLink?: string; dialIn?: string; dialInCode?: string } {
  const result: { videoLink?: string; dialIn?: string; dialInCode?: string } = {};
...
function transformEvent(event: GogEvent): CalendarEvent | null {
  ...
  // Organizer
  if (event.organizer && !event.organizer.email?.includes(event.attendees?.find(a => a.self)?.email || '')) {
...
function fetchCalendar(options: { today?: boolean; tomorrow?: boolean; week?: boolean } = { today: true }): CalendarEvent[] {
  let flags = '--today';
...
export { fetchCalendar, transformEvent, CalendarEvent };
